import { Transmission } from "./modules/transmission.mjs";
import { Color, ColorMixer, ImageDrawer, WrapEngine, ObtainableImage } from "./modules/art.mjs";
...
class LyricalPlayer {
    ...
    static getLyricalState(jadeLyrics, timePosition) {
        ...
        else if (nearestLyricalLine.startTime > timePosition) {
            currentTimeIndex--;
        }
...
class DynamicPrimaryColorEngine {
    ...
    static measureColorOpposition(c1, c2) {
        return DynamicPrimaryColorEngine.measureRelvance(c1, new Color(255 - c2.r, 255 - c2.g, 255 - c2.b));
    }
    static foregroundParameter = {
    ...
    static getPrimaryColors(pixelHarvestingMethod, width, height, parameters) {
        class ColorArrange {
    ...
    ;
    static processImageUsingAlgorithm(image, width, height) {
        ...
        function harvestFunction(x, y) {
            // properlly optimized for chromium based browsers
            let index = (Math.floor(x) + Math.floor(y) * width) * 4;
            return new Color(compiledImageData.at(index), compiledImageData.at(index + 1), compiledImageData.at(index + 2), compiledImageData.at(index + 3));
...
class AnimationTween {
    static simpleExponential(x) {
        return Math.pow(x, 2);
    }
    static fuckYouWhore(x) {
...
class DrawingRunTime {
    ...
    static characterTiming = [];
    static previousTimingState = 0;
    static displayMode = "Landscape";
    static currentAction = "Skip Track";
    static timeSinceCurrentAction = 0;
    static averageFrameRate = 0;
    static timeSinceTrackSwitchAction = 0;
    static trackSwitchAction = "left";
    static frameRateHistory = [];
    ...
    static produceMinuteShakes(index) {
        return Math.floor(5 * Math.pow(DrawingRunTime.renderObjects.DFTContent.currentAudioSample[0] || 0, 2) * Math.cos(index + Date.now() / 500)
            + 6 * Math.pow(DrawingRunTime.renderObjects.DFTContent.currentAudioSample[2] || 0, 2) * Math.cos(index + Date.now() / 50)
            + 3 * Math.pow(DrawingRunTime.renderObjects.DFTContent.currentAudioSample[5] || 0, 2) * Math.cos(index + Date.now() / 250)) *
            (BackgroundTasks.jadeLyricsSupported ? 0.75 : 1.25);
    }
    static render() {
        ...
        }
        {
            ...
            actualX += cBP.x - cBP.timePatternBoxWidth / 2;
            {
                ...
                }
                {
                    ...
                    // context2D.lineWidth = 3;
                    {
                        ...
                        context2D.moveTo(playingCoordinate[0] + (pausedCoordinate[0] - playingCoordinate[0]) * animationFactor, playingCoordinate[1] + (pausedCoordinate[1] - playingCoordinate[1]) * animationFactor);
                        for (let i = 1; i < 4; i++) {
                            let playingCoordinate = leftSegment.playing[i];
                            let pausedCoordinate = leftSegment.paused[i];
                            context2D.lineTo(playingCoordinate[0] + (pausedCoordinate[0] - playingCoordinate[0]) * animationFactor, playingCoordinate[1] + (pausedCoordinate[1] - playingCoordinate[1]) * animationFactor);
                        }
                        context2D.closePath();
                        context2D.fillStyle = backgroundColor.toStyle();
            ...
            }
            {
                ...
                if (animationFactor > 0) {
                    let timeFactor3 = Math.min(1, (Date.now() - dftContent.timeSinceAudioSample) / 250);
                    ...
                    for (let i = 0; i < 50; i++) {
                        ...
                        context2D.beginPath();
                        context2D.roundRect(-actualWidth / 2 + dftContent.width / 2 + dftContent.x + barWidth * i, dftContent.height * 0.9 + dftContent.y, barWidth, loudness * -dftContent.height * .8 * animationFactor, 10);
                        context2D.closePath();
                        context2D.fill();
                    }
                }
            ...
            }
            {
                ...
                let lyricalState = undefined;
                if (BackgroundTasks.currentSpotifyState && BackgroundTasks.currentJadeLyrics) {
                    let tPDE = cBP.timePositionDragElement;
                    currentTimePosition = tPDE.currentTimePosition - 0.1; //- .05;
                    // if (dftContent.enabled == false){
    ...
    static updateWindowSize() {
        ...
        let displayMode = DrawingRunTime.maxWidth < 1000 ? "Portrait" : "Landscape";
    ...
    static init() {
        ...
        requestAnimationFrame(DrawingRunTime.render);
        setInterval(() => {
            console.log(`Desired Frame Rate: ${DrawingRunTime.frameRate}\nSystem Capable Frame Rate: ${Math.round(1000 / DrawingRunTime.intervalFrameDelay)}\nAverage Render Time: ${Math.round(DrawingRunTime.averageRenderTime * 100) / 100}ms\nAverage Calculation Time: ${Math.round(DrawingRunTime.averageCalculationTime * 100) / 100}ms`);
        }, 250);
        let body = document.body;
        let accumulativeValues = (() => { let a = []; for (let i = 0; i < 50; i++)
            a.push(0); return a; })();
    ...
    static init() {
                            ...
                            newImage.onload = () => {
                                ...
                                }
                                {
                                    ...
                                    for (let color of results.foregroundColor) {
                                        ...
                                        let newFeasibilityFactor = Math.pow(color.fitness, 2) *
                                    ...
                                    if ((DynamicPrimaryColorEngine.measureGreyLevel(DrawingRunTime.foregroundColor) > 0.9 &&
                                        ...
                                        for (let color of results.foregroundColor) {
                                            ...
                                            //     Math.pow(DynamicPrimaryColorEngine.measureVibrance(color.color), 0) / 
                                            //     Math.pow(DynamicPrimaryColorEngine.measureRelvance(color.color, DrawingRunTime.backgroundColor), 14);
                                            let newFeasibilityFactor = Math.pow(color.fitness, 1.5) *
                                                Math.pow(Math.sqrt(color.counts), .05) *
                                                Math.pow(DynamicPrimaryColorEngine.measureBrightness(color.color), 0) *
        ...
        transmission.on("transmit", () => {
            let controller = transmission.controller;
            {
                ...
                //     }, 4);
                controller.listenMessage("DFTResults", (response, data) => {
