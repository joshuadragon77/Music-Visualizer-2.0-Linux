...
import { Color, ColorMixer } from "./art.js";
let mainConsole = console;
let timeSinceInitialization = Date.now();
let debugLogs = [];
function printToConsole(content, onlyLog = true) {
    if (onlyLog) {
...
function getScriptNameTrace(specificStack) {
...
function toNearTimeFormat(seconds) {
...
function formatOutput(currentTime, callerScript, printMode, item, format = true) {
    let color;
    let printSymbol = "";
    switch (printMode) {
        case "error": {
...
function paintAndInspectOutput(output, color) {
...
export function debugDetailed(...args) {
    if (isWebBrowser) {
        return;
    }
...
export function createPromiseTracker(func) {
...
let bindedExitFunction = [];
export function bindToExit(func) {
    bindedExitFunction.push(func);
}
if (isWebBrowser == false) {
    process.stdin.resume();
    function reportExitCode(exitCode, selfExit = false) {
        let currentTime = (Date.now() - timeSinceInitialization) / 1000;
        if (exitCode >= 128) {
...
//# sourceMappingURL=consolescript.mjs.map